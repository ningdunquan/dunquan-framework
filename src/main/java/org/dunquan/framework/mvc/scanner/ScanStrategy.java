...
public abstract class ScanStrategy {
    ...
    protected ScanStrategy(String packageName) {
    ...
    public final List<Class<?>> getClassList() {
        ...
        try {
            // 从包名获取 URL 类型的资源
            Enumeration<URL> urls = ReflectionUtil.getClassLoader().getResources(packageName.replace(".", "/"));
            // 遍历 URL 资源
...
//            logger.error("获取类出错！", e);
        	throw new RuntimeException("scan class error", e);
        }
        return classList;
    }
...
//            logger.error("添加类出错！", e);
        	throw new RuntimeException("add class error", e);
        }
    }

    private void doAddClass(List<Class<?>> classList, String className) {
        // 加载类
        Class<?> cls = ReflectionUtil.loadClass(className);
        // 判断是否可以添加类
